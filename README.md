...
```
Beware that, depending on your GPU setup, pytorch installation might need to be adapted. 
...
To train a fingeprinted model, you need to run
```
...
This will generate `.jsonl` files. Depending on the strength of the fingerprint, you may need to concatenate query and then run the watermark detector on the concatenated queries. To do so, call:
```
